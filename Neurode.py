...
class MultiLinkNode:
    ...
    def __str__(self):
        ...
        """
        upstream_id = [
            str(id(node)) for node in self._neighbors[MultiLinkNode.Side.UPSTREAM]
            ]
        downstream_id = [
            str(id(node)) for node in self._neighbors[MultiLinkNode.Side.DOWNSTREAM]
            ]
    ...
    def _process_new_neighbor(self, node: MultiLinkNode, side: Side):
        """Handle new neighboring node when added, and require implementation"""
        pass
...
class Neurode(MultiLinkNode):
    ...
    def __init__(self):
        """Initialize MultiLinkNode att's, current node value, UPSTREAM weight"""
        super().__init__()
        self._value = 0
        self._weights = {}
    ...
    def _check_in(self, node: Neurode, side: Side):
        ...
        self._reporting_nodes[side] = self._reporting_nodes[side] | 1 << node_index
